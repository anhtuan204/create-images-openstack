...
```
Lưu ý: nếu đã có channel thì phải lưu ý port.
...
Copy `/virtio-win-0.1.1/Baloon/2k12R2/amd64` từ CD Drive vào `C:\` (Lưu ý: Đường dẫn này sẽ ăn vào services Baloon vì vậy phải copy đúng)
...
- Có thể thay đổi đường dẫn service Baloon (không khuyến khích):
   regedit > HKEY_LOCAL_MACHINE > SYSTEM > CurrentControlSet > Service > Baloon > Path to Exucute > Edit > Reboot

### 2.2.2 Cài đặt Baloon driver cho CPU:
Other device > PCI Other .. > Update driver > trỏ về thư mục Balloon
...
--progress
```
hw_qemu_guest_agent + os_type phục vụ Nova Set-Password sau này.
...
### 3.5 Check lists:
- Balloon: nhận trong service, đường dẫn excute đúng, nhận trong PCI.
- QEMU-GA: nhận trong service, nhận trong service check
- Time Zone: auto, Ha Noi
- Cloubase init running, khi mở Img ra đóng lại sure là đã cài lại CloudBase Init để sau khách vào auto create password
- Firewall off + thêm rule allow remote
- Bật remote + allow insecur
- Lưu ý khi reboot lại các service trên phải Running
